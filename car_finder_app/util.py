...
def get_country_code(country_name):
    try:
        g = geocoder.geonames(country_name, key=os.environ.get('GEO_TOKEN', None))
...
def get_city_coord(city_name, country_code):
    try:
        g = geocoder.geonames(city_name, country=[country_code], key=os.environ.get('GEO_TOKEN', None))
        return g.latlng
    except Exception as e:
